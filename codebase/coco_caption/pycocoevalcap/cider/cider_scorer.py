...
class CiderScorer(object):
    ...
    def refresh_sets(self):
        ...
        self.ctest = []
        self.img_ids = []
        print('ngrams flushed, ready for more...')
...
#         print('2nd {}'.format(max(self.document_frequency.values())))
    ...
    def compute_score(self, df_mode, doc_freq, ref_len, option=None, verbose=0):
        # compute idf
        if df_mode == "corpus":
...
#             f = open(os.path.join(p, 'data', df_mode + '.p'), 'rb')
#             docFreq = pickle.load(f, encoding="latin1")
#             docFreq = pickle.load(open(os.path.join(p, 'data', df_mode + '.p'), 'r'))
        ...
        # debug
        # print score
        return np.mean(np.array([sc['score'] for sc in score])), np.array(score)
